...
import com.example.velocity.easybus.repository.UnauthorizedBookingRepository;
...
public class UnauthorizedBookingController {
	...
	@Autowired
	private UnauthorizedBookingRepository ubrepo;
	...
	public Booking saveBooking(@PathVariable(value="id") String bId,@Validated @RequestBody Booking b) throws ResourceNotFoundException
	{
		...
		UnauthorizedBooking bookd=new UnauthorizedBooking();
	...
	{
		UnauthorizedBooking book=ubrepo.findById(bId).
	...
	//GET: http://localhost:9090/bus/unauthorize/latestbooking
		...
		public UnauthorizedBooking getLatestBooking()
		{
			List<UnauthorizedBooking> allBooking=ubrepo.findAll();  //invokes findAll() method of Mongo repository
			System.out.println(allBooking);
			int temp=allBooking.size();
			System.out.println(allBooking.get(temp-1));
			UnauthorizedBooking latest=allBooking.get(temp-1);
			return latest;
